...
library(dplyr)
library(ggplot2)
get_proportions_df <- function(seurat_object, fcoex_object, algorithm_name="fcoex") {
  module_idents <- data.frame(fcoex_object@mod_idents)
  colnames(module_idents) <- paste(algorithm_name, colnames(module_idents), sep="_")
  ...
  original_clusters <- data.frame(table(Idents(seurat_object)))
  seurat_to_fcoex_proportions = list()
  
  for (identity in levels(Idents(seurat_object))){
...
prepare_proportions_df <-function(proportions_df){
...
plot_proportions_df <- function(proportions_df){
  proportions_df %>%
