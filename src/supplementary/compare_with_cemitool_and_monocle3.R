...
annot <-data.frame(SampleName = colnames(exprs), Class = meta$cell_id)
...
cem <- cemitool(data.frame(exprs), annot, gmt=gmt_in,
                interactions=int_df, verbose=TRUE, plot=TRUE)

gene_module_df_clean <- cem@module
gene_module_df_clean$modules <- as.factor(gene_module_df_clean$modules)

cemitool_modules <- list()
for (module_name in levels(gene_module_df_clean$modules)){
  module_genes <- gene_module_df_clean[gene_module_df_clean$modules == module_name, ]$genes
...
for (i in levels(gene_module_df_clean$module)){
  module_genes <- gene_module_df_clean[gene_module_df_clean$module == i, ]$id
  module_name = paste0("M", i)
  monocle3_modules[[module_name]] <- module_genes
}

saveRDS(monocle3_modules, file = "../../outputs/other/pbmc3k_modules_monocle3.rds")



# Compare the outputs of the 3 methods
